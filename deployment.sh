...
updateConfigMapAndIngress() {
  echo "📦 Updating im discover configMap YAML..."
  kubectl apply -f deployments/deploy/discover-config.yaml -n "$ENVIRONMENT"

  echo "📦 Updating im discover traefik ingress route YAML..."
...
applyCronjobs() {
  ...
  for cron in deployments/cronjobs/*.yaml; do
    if [[ -f "$cron" ]]; then
      ...
      IMAGE_NAME="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/akachat-v2/im-discover:${SHORT_ENV}-publisher-${VERSION}"
...
buildAndPush() {
  ...
  for service in "${images[@]}"; do
    ...
    IMAGE_NAME="${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/akachat-v2/im-discover:${SHORT_ENV}-${SHORT_NAME}-${VERSION}"
